...
import countries from "./globo.json";
...
export function Globe({ globeConfig, data }: WorldProps) {
    ...
    const _buildData = () => {
        ...
        for (let i = 0; i < arcs.length; i++) {
            ...
            if (!rgb) {
                console.error(`Invalid color provided: ${arc.color}`);
                continue;
            }
            points.push({
    ...
    useEffect(() => {
        if (globeRef.current && globeData && countries) {
            console.log("Todos los valores están definidos, ejecutando configuración del globo.");
            globeRef.current
    ...
    const startAnimation = () => {
        ...
        console.log("Iniciando animación de arcos", data); // Debugging
        globeRef.current
            .arcsData(data)
